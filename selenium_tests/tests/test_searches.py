...
class TestSearches(TestBase):
    ...
    @pytest.mark.parametrize("fake_search", [ "dsfsdgdfgsdfgsdfgsdfgertrgdffgdgsdfghdfgdhdghgfdhgh", "dsfsdgdfgsdfgsdfgsdfgertrgdffgdgsdfghdfgdhdghgfdhgh4343"])
    def test_fake_search_returns_nothing(go_to_base_page: BasePage, fake_search: str):
        ...
        search_page = base_page.search(fake_search)
        error_msg = "Some results are found even after using the random string '{}'".format(fake_search)
        assert "did not match any documents" in search_page.error_notification_text, error_msg
    ...
    #     """
    #     base_page = go_to_base_page
    #     search_page = base_page.search_empty(" ")
    #
    #     assert 0 == search_page.results_counter_text
